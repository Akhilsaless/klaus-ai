...
export const appRouter = router({
  ...
  tasks: router({
    ...
    getOutputs: protectedProcedure.input(z.object({ taskId: z.number() })).query(async ({ ctx, input }) => {
      const task = await getTaskById(input.taskId);
      if (!task || task.userId !== ctx.user.id) throw new Error("Task not found");
      return getOutputsByTaskId(input.taskId);
    }),
    getLogs: protectedProcedure.input(z.object({ taskId: z.number() })).query(async ({ ctx, input }) => {
      const task = await getTaskById(input.taskId);
      if (!task || task.userId !== ctx.user.id) throw new Error("Task not found");
      return getLogsByTaskId(input.taskId);
    }),
    create: protectedProcedure.input(z.object({ goal: z.string().min(1).max(2000) })).mutation(async ({ ctx, input }) => {
      ...
      await createChatMessage({ taskId: task?.id, userId: ctx.user.id, role: "user", content: input.goal });
      return task;
    }),
  ...
  chat: router({
    ...
    sendMessage: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) =>
        createChatMessage({ userId: ctx.user.id, taskId: input.taskId, role: "user", content: input.content })
      ),
  }),
  ...
  agents: router({
    ...
    delegationPreview: protectedProcedure
      .input(z.object({ steps: z.array(z.object({
        stepIndex: z.number().int().min(0),
        title: z.string().min(1).max(512),
        description: z.string().max(2000).optional(),
        tool: z.enum(["web_research", "code_generator", "file_generator", "email_generator", "data_analysis", "text_writer"]),
      })).min(1).max(20) }))
      .query(({ input }) => buildDelegationPlan(input.steps)),
    policyPreview: protectedProcedure
      .input(z.object({
        agentId: z.enum(["executive", "computer", "mobile", "research", "communication", "finance", "project", "developer", "security"]),
  ...
  aiAdmin: router({
    ...
    routePreview: superAdminProcedure
      .input(z.object({
        ...
        taskClass: z.enum(["routine", "classification", "extraction", "summarization", "simple_planning", "complex_planning", "critical_verification", "computer_use", "premium_voice"]).optional(),
        ...
        requireCapabilities: z.array(z.enum(["text", "json", "reasoning", "verification", "computer_use", "realtime_voice"])).max(6).optional(),
      }))
      .query(({ input }) => decideProvider(input.prompt, {
    ...
    upsertProviderConfig: superAdminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await createAuditEvent({
          ...
          risk: "medium",
          result: "allowed",
          metadata: { enabled: input.enabled, model: input.model ?? null, hasSecretRef: Boolean(input.secretRef) },
        });
        return { success: true } as const;
      }),
